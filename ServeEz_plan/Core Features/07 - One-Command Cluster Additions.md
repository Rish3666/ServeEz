...
- `servez join <url> --token=xxx` bootstrap command (`cmd/servez/join.go`).
- Token-based registration → `/v1/nodes/register` → agent boots and registers the node.
- Join token auto-generated by the control plane on first boot (`ensureJoinToken`).
- See [[Obsidian/04 - Agent Runtime]] for the node-side flow.
