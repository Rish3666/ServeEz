...
No SSH, no raw `kubectl apply -f`. Every operation — deploy, scale, heal, migrate, configure — is exposed through a structured API (REST/gRPC/MCP). The AI never reads or writes config files directly. YAML is generated by AI as a record (Helm-style), and humans can edit it as an override path.
