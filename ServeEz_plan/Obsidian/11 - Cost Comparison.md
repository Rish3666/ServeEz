...
- [[internal/cost/cost.go]]: `Engine` implements `api.CostComparer`; offline baseline 2026 catalog (representative per-provider pricing) + `Compare`.
...
- `servez cost --vcpu=N --mem-gb=M [--runtime-pct] [--region]` — CLI command ([[cmd/servez/cost.go]]).
- `apiclient.CostCompare` ([[internal/apiclient/client.go]]).
